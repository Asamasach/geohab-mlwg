"""Stronger CNN: wider arch, more epochs, 2-seed bagging, 16-way TTA, mixup augmentation."""
import sys
...
class WiderCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fc(torch.cat([a, m], dim=1))
...
for seed in SEEDS:
    ...
    for fold, (tr_i, va_i) in enumerate(skf.split(X, y)):
        ...
        for ep in range(EPOCHS):
            ...
            with torch.no_grad():
                for xb, yb in va_dl:
                    all_p.append(F.softmax(model(xb.to(DEVICE)), dim=1).cpu().numpy())
        ...
        vb = []
        for xb, _ in va_dl:
            vb.append(tta16(model, xb.to(DEVICE)))
        oof[va_i] += np.vstack(vb) / len(SEEDS)
        tb = []
        for xb in te_dl:
            tb.append(tta16(model, xb.to(DEVICE)))
        pte += np.vstack(tb) / (N_FOLDS * len(SEEDS))
        print(f"  fold {fold} best macroF1={best_f1:.4f}", flush=True)

print(f"\nCNNv2 OOF macroF1: {f1_score(y, oof.argmax(1), average='macro'):.4f}")
print(f"CNNv2 OOF weightedF1: {f1_score(y, oof.argmax(1), average='weighted'):.4f}")
print(classification_report(y, oof.argmax(1), target_names=classes, digits=4))

np.save(DATA / "oof_cnn_v2.npy", oof)
np.save(DATA / "pte_cnn_v2.npy", pte)
sub = pd.DataFrame({"ID": test["ID"].values, "class": [i2c[i] for i in pte.argmax(1)]})
sub.to_csv(DATA / "sub_cnn_v2.csv", index=False)
print(f"\nsub_cnn_v2.csv dist: {sub['class'].value_counts().to_dict()}")