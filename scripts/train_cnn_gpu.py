"""GPU CNN for GeoHab. Spatial-aware training:
- Wider arch + mixup + 16-way TTA + 3-seed bagging
- Spatial cross-validation for model selection
- Uses random 5-fold for final predictions (matches competition setup)"""
import sys
import os
...
class WiderCNN(nn.Module):
    def __init__(self, nc=5):
        ...
        self.net = nn.Sequential(
            blk(2, 32),   # 64 -> 32
            blk(32, 64),  # 32 -> 16
            blk(64, 128, drop=0.1),  # 16 -> 8
            blk(128, 192, drop=0.1), # 8 -> 4
        )
...
for seed in SEEDS:
    ...
    for fold, (tr_i, va_i) in enumerate(skf.split(X, y)):
        ...
        print(f"  fold {fold} seed {seed} best macroF1={best_f1:.4f} t={time.time()-t0:.0f}s", flush=True)

print(f"\nCNN-GPU OOF macroF1: {f1_score(y, oof.argmax(1), average='macro'):.4f}")
print(f"CNN-GPU OOF weightedF1: {f1_score(y, oof.argmax(1), average='weighted'):.4f}")
print(classification_report(y, oof.argmax(1), target_names=classes, digits=4))

np.save(DATA / "oof_cnn_gpu.npy", oof)
np.save(DATA / "pte_cnn_gpu.npy", pte)
sub = pd.DataFrame({"ID": test["ID"].values, "class": [i2c[i] for i in pte.argmax(1)]})
sub.to_csv(DATA / "sub_cnn_gpu.csv", index=False)
print(f"\nsub_cnn_gpu.csv dist: {sub['class'].value_counts().to_dict()}")