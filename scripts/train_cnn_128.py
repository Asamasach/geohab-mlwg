"""Larger-context CNN: 128x128 patches (32m×32m). ResNet-style, 3-seed bag, TTA."""
import sys, os
...
X = np.load(DATA / "patches_train_128.npy")
Xte = np.load(DATA / "patches_test_128.npy")
    ...
    def __getitem__(self, i):
        ...
        if self.augment:
            ...
            if np.random.rand() < 0.5:
                x = x[:, ::-1, :].copy()
            if np.random.rand() < 0.5:
                sh, sw = np.random.randint(-12, 13, size=2)
                x = np.roll(x, (sh, sw), axis=(1, 2))
            if np.random.rand() < 0.4:
                x = x + np.random.randn(*x.shape).astype(np.float32) * 0.08
            # Random crop
            if np.random.rand() < 0.3:
                cs = np.random.randint(96, 128)
                y0 = np.random.randint(0, 128 - cs + 1)
                x0 = np.random.randint(0, 128 - cs + 1)
                c = x[:, y0:y0+cs, x0:x0+cs]
                c = torch.from_numpy(np.ascontiguousarray(c)).unsqueeze(0)
                c = F.interpolate(c, size=(128, 128), mode="bilinear", align_corners=False).squeeze(0).numpy()
                x = c.copy()
...
class Res128(nn.Module):
    def __init__(self, nc=5):
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv2d(2, 32, 5, stride=2, padding=2, bias=False),  # 128 -> 64
            nn.BatchNorm2d(32), nn.ReLU(inplace=True),
            nn.MaxPool2d(2),                                        # 64 -> 32
        )
        self.layer1 = nn.Sequential(BasicBlock(32, 64, 2), BasicBlock(64, 64))      # 32 -> 16
        self.layer2 = nn.Sequential(BasicBlock(64, 128, 2), BasicBlock(128, 128))   # 16 -> 8
        self.layer3 = nn.Sequential(BasicBlock(128, 256, 2), BasicBlock(256, 256))  # 8 -> 4
        self.layer4 = nn.Sequential(BasicBlock(256, 384, 2), BasicBlock(384, 384))  # 4 -> 2
...
for seed in SEEDS:
    ...
    for fold, (tr_i, va_i) in enumerate(skf.split(X, y)):
        ...
        vb = []
        for xb, _ in va_dl:
            vb.append(tta8(model, xb.to(DEVICE, non_blocking=True)))
        oof[va_i] += np.vstack(vb) / len(SEEDS)
        tb = []
        for xb in te_dl:
            tb.append(tta8(model, xb.to(DEVICE, non_blocking=True)))
        pte += np.vstack(tb) / (N_FOLDS * len(SEEDS))
        print(f"  fold {fold} seed {seed} best={best_f1:.4f} t={time.time()-t0:.0f}s", flush=True)

print(f"\n128-CNN OOF macroF1: {f1_score(y, oof.argmax(1), average='macro'):.4f}")
print(classification_report(y, oof.argmax(1), target_names=classes, digits=4))
np.save(DATA / "oof_cnn_128.npy", oof)
np.save(DATA / "pte_cnn_128.npy", pte)
sub = pd.DataFrame({"ID": test["ID"].values, "class": [i2c[i] for i in pte.argmax(1)]})
sub.to_csv(DATA / "sub_cnn_128.csv", index=False)
print(f"\nsub_cnn_128.csv dist: {sub['class'].value_counts().to_dict()}")